        ...
        <table class="table table-bordered table-striped table-hover">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td> {{ $piquete->capim }} </td>
                    <td>
                        <a href="{{ url("frontEnd/talhao/{$talhao->id}/piquete/{$piquete->id}/edit") }}" class="btn btn-primary btn-xs">Editar</a>
                        {!! Form::open([
                            'method'=>'DELETE',
                            'url' => ['frontEnd/piquete', $piquete->id],
                            'style' => 'display:inline'
                        ]) !!}
                            {!! Form::submit('Deletar', ['class' => 'btn btn-danger btn-xs']) !!}
                        {!! Form::close() !!}
                        <a href="{{ url("frontEnd/talhao/{$talhao->id}/piquete/{$piquete->id}") }}" class="btn btn-success btn-xs">Mostrar</a>
                    </td>
                </tr>
                @endforeach
            </tbody>
